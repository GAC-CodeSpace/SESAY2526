...
public class Bestiary {
    ...
    static {
        ...
        dungeonMinions.put(1, Arrays.asList(
            new EnemyTemplate("Scheletro", "skeleton", 50)
        ));
        // Boss: Scheletro Gigante (versione più debole)
        dungeonBosses.put(1, new EnemyTemplate("Scheletro Gigante", "giant_skeleton", 200));

        // --- LIVELLO 2 ---
        // Minions: Scheletri più forti
        dungeonMinions.put(2, Arrays.asList(
            new EnemyTemplate("Scheletro Guerriero", "skeleton_warrior", 80)
        ));
        // Boss: Scheletro Gigante (versione più forte)
        dungeonBosses.put(2, new EnemyTemplate("Scheletro Gigante Antico", "giant_skeleton", 400));
    }

    public static EnemyTemplate getMinionForLevel(int level) {
        List<EnemyTemplate> list = dungeonMinions.get(level);
        if (list == null || list.isEmpty()) return new EnemyTemplate("Scheletro Base", "skeleton", 30);
        return list.get(random.nextInt(list.size()));
    }

    public static EnemyTemplate getBossForLevel(int level) {
        EnemyTemplate boss = dungeonBosses.get(level);
        if (boss == null) return new EnemyTemplate("Scheletro Gigante", "giant_skeleton", 150);
        return boss;
    }
}
