...
public class PlayerCombatant extends Combatant {
    ...
    private void initializeSkills(String archetype) {
        if (archetype.equalsIgnoreCase("Warrior")) {
            ...
            this.skills.add(new Skill("Fendente", 0, true,(u, t, ctx) -> t.takeDamage(u.getAttackPower())));

            // 2. Provocazione: Forza il nemico ad attaccare il tank per 2 turni
            this.skills.add(new Skill("Provocazione", 3, sourceData.getKarma()>=20,(u, t, ctx) -> t.addStatusEffect(new StatusEffect("TAUNT", 2, u))));
            ...
            this.skills.add(new Skill("Rottura Guardia", 3, sourceData.getKarma()>=55,(u, t, ctx) -> t.addStatusEffect(new StatusEffect("VULNERABLE", 1, 1.5f))));

// 5. Ultimo Baluardo (Fix: Forza il calcolo dei danni)
            this.skills.add(new Skill("Ultimo Baluardo", 5, sourceData.getKarma()>=80,(u, t, ctx) -> {
            ...
            this.skills.add(new Skill("Dardo Arcano", 0, true,(u, t, ctx) -> t.takeDamage(u.getAttackPower())));

            // 2. Congelamento: Danno basso + Salta turno (STUN)
            this.skills.add(new Skill("Congelamento", 3, sourceData.getKarma()>=20,(u, t, ctx) -> {
                t.takeDamage(5);
                t.addStatusEffect(new StatusEffect("STUN", 1));
            }));

// 3. Tributo di Sangue (Fix: Aggiunto danno reale)
            this.skills.add(new Skill("Tributo di Sangue", 4, sourceData.getKarma()>=35,(u, t, ctx) -> {
            ...
            this.skills.add(new Skill("Scudo Magico", 4, sourceData.getKarma()>=55,(u, t, ctx) ->
                u.addStatusEffect(new StatusEffect("SHIELD_REFLECT", 1, 0.5f, t))
            ));
// 5. Supernova (Fix: Danno ad area funzionante)
            this.skills.add(new Skill("Supernova", 6, sourceData.getKarma()>=80,(u, t, ctx) -> {
